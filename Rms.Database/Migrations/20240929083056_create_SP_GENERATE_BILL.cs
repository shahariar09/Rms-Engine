...
{
    /// <inheritdoc />
    public partial class create_SP_GENERATE_BILL : Migration
    {
        /// <inheritdoc />
		...
		--cu.WaterBill,
		CASE WHEN (ISNULL(IsWaterBillRequire,0)=1) THEN '0'
        ELSE
		ISNULL((SELECT CAST(wbg.BillMonthTotal AS NVARCHAR(50))  FROM RMS_GASS_BILL wbg WHERE wbg.CustomerId =  @CustomerId AND MONTH(IssueDate) = MONTH(@formate) AND YEAR(IssueDate) =YEAR(@formate)),-1) END ,
		cu.OtherBill,
		CASE WHEN (ISNULL(cu.IsGasBillRequire,0)=1) THEN '0'
		ELSE
	    ISNULL((SELECT CAST(gbg.BillMonthTotal AS NVARCHAR(50))   FROM RMS_GASS_BILL gbg WHERE gbg.CustomerId =  @CustomerId AND MONTH(IssueDate) = MONTH(@formate) AND YEAR(IssueDate) =YEAR(@formate)),-1) END
